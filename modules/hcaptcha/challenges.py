...
class Challenge:
    ...
    def solve(self, answers):
        ...
        resp = self._request(
            method="POST",
            url=f"{self.base_url}/checkcaptcha/{self.sitekey}/{self.key}",
            headers={
                "Host": "hcaptcha.com",
            ...
            data=json.dumps({
                "answers": {
                    task.key: {"text":"yes"} if task.key in answers or task in answers \
                              else {"text":"no"}
    ...
    def _simulate_solve(self, answers):
        ...
        for page in range(total_pages):
            ...
            btn_pos = (
                ...
                VERIFY_BTN_POS[0] + random.randint(5, 15),
                VERIFY_BTN_POS[1] + random.randint(5, 20),
            )
